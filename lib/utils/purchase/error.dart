...
Exception? mapToDisplayedException(PlatformException exception) {
  ...
  switch (errorCode) {
    ...
    case PurchasesErrorCode.productNotAvailableForPurchaseError:
      ...
      return const FormatException('対象のプランは現在販売しておりません。お手数ですがアプリを再起動の上お試しください');
    case PurchasesErrorCode.productAlreadyPurchasedError:
      ...
      return const FormatException('すでにプランを購入済みです。この端末で購入情報を復元する場合は「以前購入した方はこちら」から購入情報を復元してくさい');
    ...
    case PurchasesErrorCode.missingReceiptFileError:
      return FormatException('購入者の情報が存在しません。$accountName で端末にサインインをした上でお試しください');
    case PurchasesErrorCode.networkError:
      return const FormatException('ネットワーク状態が不安定です。接続状況を確認した上でお試しください。');
    ...
    case PurchasesErrorCode.operationAlreadyInProgressError:
      return const FormatException('購入処理が別途進んでおります。お時間をおいて再度ご確認ください');
    ...
    case PurchasesErrorCode.insufficientPermissionsError:
      return FormatException('お使いの $accountName ではプランへの加入ができません。お支払い情報をご確認の上再度お試しください');
    case PurchasesErrorCode.paymentPendingError:
      return FormatException('支払いが途中で止まっております。ログイン中の$accountNameで$storeNameをお確かめくだい');
